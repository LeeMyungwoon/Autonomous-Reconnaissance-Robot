...
# 🤖 Patrol Robot – 정찰 로봇 프로젝트 (Team Project)

이 프로젝트는 영상/음성/딥러닝 기반 기술을 활용해 **자율 순찰 및 위험 상황 인식이 가능한 스마트 로봇 시스템**을 개발한 4인 팀 프로젝트입니다.  
본인은 **장애물 탐지 및 회피 알고리즘**과 관련된 딥러닝 모델 구현을 담당하여 로봇의 안정적인 자율 주행 성능을 확보하는 데 핵심적인 기여를 하였습니다.

🔗 원본 팀 프로젝트 레포지토리: [addinedu-ros-8th/deeplearning-repo-2](https://github.com/addinedu-ros-8th/deeplearning-repo-2)
...
| 역할 구분                | 상세 내용 |
|---------------------------|-----------|
| 🚧 장애물 인식 모델 개발     | - YOLO 기반 객체 인식 모델로 장애물 탐지 수행<br> - 정적/동적 장애물 구분을 위한 커스텀 클래스 설계 |
| 🌀 회피 알고리즘 구현       | - 탐지된 장애물 정보를 기반으로 우회 경로 결정 알고리즘 구현<br> - 움직이는 대상(사람)과의 충돌 회피 로직 반영 |
| 📐 Depth 추정 모델 적용    | - Stereo Vision 기반 Depth 추정 적용<br> - 거리 정보 기반 회피 전략 보완 및 실시간 주행 적용 |
